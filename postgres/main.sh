...
function benchmark_run() {
  ...
  psql -h $HOST -p $PORT  -d $DBNAME -f ${ROOT}/primary_key.sql> $RESULTS/primary_key.log 2> $RESULTS/primary_key.err

  print_log "  creating foreign keys"
  psql -h $HOST -p $PORT  -d $DBNAME -f ${ROOT}/foreign_key.sql > $RESULTS/foreign_key.log 2> $RESULTS/foreign_key.err
  ...
  print_log "  analyzing"
  psql -h $HOST -p $PORT  -d $DBNAME -c "analyze" > $RESULTS/analyze.log 2> $RESULTS/analyze.err
